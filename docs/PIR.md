...
关键的想法是自动并行化可用的计算能力，将其划分为几个较小的委员会，每个委员会处理一组不相交的事务（或分片）。委员会的数量与网络中的总计算能力成比例增长。所有委员会，每个委员会都有一个小的常数c成员，在内部运行经典的拜占庭共识协议，以就一个价值达成一致。一个称为共识委员会的最终委员会负责组合其他委员会选择的分片，计算加密摘要并将其扩展到整个网络。作为该时期的最后一步，最终委员会生成一组共享的公共随机位串，这些位具有有界偏差。这些随机字符串在随后的时期中用作随机源 - 确保对手不能使用前一时期的观察来模拟任何不可忽略的计算能力增益。

我们介绍ELASTICO，第一个用于无权区块链的安全分片协议的候选者。 从根本上说，ELAS-TICO将协议吞吐量与网络的计算能力线性接近，并且在部分同步网络中容忍控制多达四分之一计算容量的拜占庭通道。 它在实验中提供了有前途的可扩展性，并表明在下一代加密中具有很强的可用性。
