...
接下来在我们列表中出现的是重要性证明区块链共识协议。该共识案例的由来是因为NEM的名字而闻名。该概念是在PoS的基础上发展而来的。尽管如此，但NEM引入了一个新的想法--收获或者保留退休金的权利。
...
如果你与其他NEM账户持有者进行交易的话，那么PoI算法将会奖励你。该网络会将你视为合作伙伴。如果打算伪装这种伙伴关，系系统可以发现你。
...
容量证明共识示例是著名的PoW共识协议的升级版本。该协议本质的特点之一是绘图（阴谋？）。在开始挖矿之前你将必须贡献你的计算机算力和硬盘存储。

该系统比PoW更快也是在自然之中。PoC仅仅用4分钟就可以创建一个区块，而PoW却要10分钟。而且，该系统试图解决PoW系统中的哈希问题。你的电脑上有越多的解决办法和阴谋时，你将有更好的机会来赢得挖矿对决。


#### 容量证明机制是如何工作的？

为了理解该共识定理的本质，你必须要了解两个概念--阴谋和挖掘。

通过合理安排你电脑的硬盘，你基本上可以创建一个随机数。该随机数和比特币中的随机数还有些不同。这里，你将必须计算出你的ID和数据的哈希值直到你算出这个随机数。

每个随机值捆绑着8192个哈希值。捆绑哈希值又称为挖掘。每个ID可以接收的最大的范围是4095。

下一个概念是在硬盘上挖掘。正如我们所说，你可以在同一时间收到0-4095范围内的金币并且存储在你的硬盘上。你将被分配一个最低截止日期来解决随机值。这个期限也表明了创建块的时间。
...
讲真，该区块链共识算法也有很严重的问题。首先，该过程会在硬盘上创建大量的冗余数据。该系统青睐于那些拥有足够大的存储单元的矿工对去中心化的概念构成威胁。甚至黑客也可以利用该系统或者注入恶意挖矿软件到系统中。



### Proof of Burn

该共识序列是相当令人印象深刻的。为了安全防护PoW加密货币，一部分金币将会被烧掉。当矿工将金币给食用者地址时，该过程便会发生。该食用地址无法用于任何目。账本记录这些燃烧掉的金币，使得它们完全不可以继续花费。烧掉金币的用户可以得到一份奖励。

是的，燃烧意味着失去。但是该过程在长久防护金币免受黑客的网络攻击中来说，损害是暂时的。而且，燃烧过程增加了可选择金币的赌注。
...
尽管这些金币不可访问或者永远消失了，但是他们被视为计算供应并标记为烧毁。
