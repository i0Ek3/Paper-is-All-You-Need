...
> 本文转载自[101blockchains](https://101blockchains.com)，作者是Hasib Anwar。

> 原文为[Consensus Algorithms: The Root Of The Blockchain Technology](https://101blockchains.com/consensus-algorithms-blockchain/)。

> 本文为原文的中文翻译，属个人翻译作品，翻译不到之处还请指正。也感谢Hasib Anwar写的文章。
...
如果你想开始使用PIVX，你需要下载官方的钱包并且同步区块链中的数据。在这之后，你可以向钱包里转入一些货币，然后将其保持连接状态。
...
该平台提供侧链中app的开发，可防止任何类型的网络延迟。他们开始于PoW项目，但是，最终他们转向了PoW。
...
同证人一样，代理也会被选中。代理们用于全面更改网络的参数。对于代理来说，它们可可以获得交易费用，块间隔，块大小和证人的支付。
...
在DPoS中，双花的风险在很大程度上已经降低了。当区块链网络未能在数据库中包含先前花费的事务是，可能发生这种情况。

网络可以在没有任何人帮助的情况下检查该交易是否正常，并且可以探测任何种类的损失。在这种方式下，它可以保证数据库100%透明。
...
经典的PoS的另一个转折点是LPoS--租赁的股权证明。新的区块链共识算法被引入到我们的Waves平台。就像其他区块链技术平台一样，Waves平台确保以有限的工号提供更好的挖矿。

最初的PoS对参与赌注有一些限制。拥有有限数量的个人可能永远不会真正参与赌注。为了成功维护网络，只留下少数拥有更多硬币的个人。
...
钱包里有少量金币的人永远不会有机会获得像大鱼一样的好处。这样就完全确立了共识算法的主题--透明性。


### PoET

PoET是最好的共识算法之一。该特定的算法主要用于许可区块链，你需要获取访问网络的权限。这些有权限的网路需要来决定采矿权和投票原则。

为了确保所一切顺利，PoET算法使用特定的策略来处理整个网络的透明性。共识算法也确保可以安全登录系统，因为在加入到矿工行列之前需要认证。

毋庸置疑，该共识算法有机会仅用公平的方式挑选获胜者。

让我们看看这个非常棒的共识序列的主要策略是什么。

* 每个人在网络上都需要等待一定的时间，但是对于时间的限制完去是随机的。
* 当参与者完成它所分配的等待时间便可以在账本上创建一个新的区块了。
...
PoET依赖特定的CPU，称为因特尔软件防护扩展。该SGX由助于在网络中运行独特的代码。PoET使用该系统来确保获胜是公平的。
...
对第一步是证明你真正网络上的可信代码而不是其他的什么随机技巧。如果第一步没有合适地运行，则可能永远无法找到主网络。
...
* 一个新用户首先在区块链中下载可信代码。
* 之后当TA开始该过程，他们会获得一个特殊的密钥对。
* 使用那个蜜钥对，用户可以发送SGX证明到网络并请求访问。
...
PBFT主要关注于状态机。它复制了系统但是摆脱了拜占庭将军的一般问题。现在，它是如何做的呢？

当然，该算法从一开始就假定网络中有可能存在故障，并且某些独立节点在某些时刻发生故障。
...
但是，通过这个新过程，他们能够发现即使其中的一个节点受到损害。所有的节点通过多数的投票可以达成协议。
...
生成器应用所有节点遵循的某些规则来验证所有交易。之后，块签名者将验证他们并添加他们到自己的签名中。这就是为什么如果任何一个块错过了其中一个密钥后会被拒绝的原因了。
...
SBFT用于网络中保密性占优先地位高的私有网络。该平台的设计是为可揭露敏感敏感信息，但存在一定的局限性。这就是系统使用三种技术的原因。例如零知识证明，一次性地址是使用，加密元数据。
...
链，是一个基于区块链的平台，它使用SBFT来验证网络上的所有交易。除此之外，它们还使用HSM来实现工业级安全性。通过使用HSM，它们可以确保在无需任何单个节点故障的情况下仍然安全。


### dBFT

毫无争议的事实是PoW和PoS是广为人知的共识算法。虽然很多区块链生态系统使用这两种通用的算法，但有些试图强加更新、更高级的共识系统。在这些先锋区块链品牌中，NEO的名字肯定会来。
...
NEO使用dBFT来验证所有的交易。如果你在你的NEO中参与赌注，你将能生成GAS。GAS是该平台的主要流通货币。你将必须为每笔交易支付一定量的GAS费用。这是为什么你有囤有越多的NEO，你将获得更多的GAS的原因了。
...
不要流汗？NEO向我们展示了解决拜占庭将军问题的更好方法。现在让我们看看NEO引以为傲的委托拜占庭容错共识机制。dBFT主要测重于以两种方式来解决已有模型--更好的可拓展性和更好的性能。


#### 发言人和代表们！

我们将再次使用另一个例子来理清dBFT模型。让我们考虑下拜占庭军队有一位当选的领导人而不是一位官僚将军。这位被选中的领导人将担任军队的代表。
...
dBFT区块链共识协议也我们提供了解决这些场景的方法。正如我们所说，账本将节点的决策保存在某个地方。代表们会验证发言人所描述的是否属实。如果发言人的提议和账本无法关联，66%的代表会拒绝发言人的提议并且完全禁止该发言人。


#### 不诚实的代表们

另一个场景是有一个诚实的发言人，但可能背叛了代表。这里，诚实的代表和诚实的发言人将努力成为66%的大多数，并减少不诚实的代表的努力。

那么，你可以看到dBFT是如何克服拜占庭将军的缺陷和BFT共识算法。诚然，NEO值得世界各地的赞扬，它们努力创造了一个更好的共识算法。
...
当人们在讨论PoW和PoS的话题时，莱特币的创始人和其他三位作者想到了一些非常棒的事情。他们向世界提出了一个简单的问题--为什么不能把PoW和PoS结合起来而时让它们相互争斗呢？
...
验证者持有的堆栈越多，他们批准块的可能性就会越大。只有在验证后，该特定块会被加入到区块链中。

这就是PoA，如何使用两种共识算法的最佳算法来验证区块并将其添加到区块链中。此外，网络也会付给矿工和验证者平等的份额作为交易费用。因此，该系统抵抗“共同的悲剧“，并为区块验证创建了更好的解决方案。

#### PoA的影响

区块链面对的最大的威胁之一是51%算力攻击。共识定理将51%攻击的可能性降低到0。它发生的原因是矿工和验证者都不能占多数。因为在向网络中添加快时，该过程需要相同的贡献。

尽管这样，一些批评家认为，PoA区块链共识协议有一些严重缺陷。第一个便是由采矿特性导致的大量能源消耗。第二，PoA没有任何解决方案来阻止验证者的双重签名问题。这两个重达的缺陷使得该共识定理有点落后。
...
接下来在我们列表中出现的是重要性证明区块链共识协议。该共识案例的由来是因为著名的NEM。该概念在PoS的基础上发展而来的。尽管如此，但NEM引入了新的想法--收获或者保留退休金的权利。

收获机制决定一个节点是否有资格被添加到区块链中。一个节点收获的越多，则有更多的机会被添加到链中。相反为了收获，节点接收验证者作为奖励的交易费用。当有资格去收获时，你的账户上至少要有10000个XEM。

该机制解决了PoS的主要问题。在PoS中，与那些拥有少量金币的验证者相比，富有的人拥有很多金币。例如，如果你拥有20%的加密货币，你可以在区块链网络上挖掘20%的区块。这就导致了该共识算法更青睐有钱的人。
...
该共识定理中最重要的 特性时保留退休金的权利或者收获。正如我们所说的那样，你至少需要10000个金币才有资格在开始的时候进行收获。你的PoI得分依赖于你拥有的收获账户。尽管如此，但是该共识算法

* 合伙交易

如果你与其他NEM账户持有者进行交易的话，那么PoI算法将会奖励你。该网络会将你认为是合作者。尽管，系统可以抓住你如果打算伪装这种伙伴关系。

* 评分系统

该交易在你的PoI得分中提出一个影响？分数是基于你在30天的有限期内所进行的交易。越流畅月本质的总和将会帮助你在NEM网络中提升你的分数。



### Proof of Capacity

容量证明共识算法的例子是著名的PoW共识协议的升级版本。该协议本质的特点之一是阴谋。在开始挖矿之前你将必须贡献你的计算机算力并且硬驱动存储。

该系统比PoW更快也是在自然之中。PoC仅仅用4分钟就可以创建一个区块，而PoW却要10分钟。而且，该系统在尝试解决PoW系统中的哈希问题。你的电脑上有越多的解决办法和阴谋是你将有更好的机会来赢得挖矿对决。

#### 容量证明机制是如何工作的？

为了理解该共识算法的本质，你必须要了解两个概念--阴谋和挖矿。

通过阴谋，你电脑的硬盘，可以基本的创建一个随机值。该随机值和比特币中的随机值还有些不同。这里，你将必须计算出你的ID和数据的哈希值直到你算出这个随机值。

每个随机值绑定者8192个哈希值。绑定的数字是 。每个ID可以接收的最大的范围是4095。

下一个概念是在硬盘上挖矿。正如我们所说，你可以在同一时间并且存储所接收到的在0-4095范围内的金币子啊你的硬盘上。你将被分配一个最小的工作期限来解决随机值。这个期限也表明了创建块的时间。

如果你比其他矿工更早地管理解决的随机值，你将会得到一个块作为奖励。一个著名的例子便是Burst也采用了PoC算法。


#### 容量证明的利和弊

在硬盘上挖矿比常规的PoW更为节能。你将不必花给运气来获得昂贵的挖矿 我们可以在比特币协议中见到。在该共识算法离，只要你电脑上的硬盘有足够的空间便可以挖矿。

讲真，该区块链共识算法也有很严重的问题。首先，该过程会在硬盘上创建大量的冗余数据。该系统青睐于矿工有足够大的存储单元可以提出一个威胁去中心化的概念。甚至黑客可能会利用系统或者注入恶意挖矿软件到系统中。



### Proof of Burn

该共识序列是相当令人印象深刻的。为了安全防护PoW加密货币，该金币的比例将会被烧掉。改过程发生在矿工发送一些金币给 地址。该 地址无法以任何目的再次发送这些金币。一个账本记录这些燃烧掉的金币使得他们正真的不可花掉。烧掉金币的用户可以得到一份奖励。

是的，燃烧意味着失去。但是该过程在长久防护金币免受黑客的网络攻击中来说，损害是临时的。而且，燃烧过程增加了可选择金币的赌注？

有这样一个场景增加了用户在下一个区块挖矿的机会，也增加了他们在未来的奖励。因此，燃烧可以被用于挖矿优先级中。在加密货币中相应的部分是一个非常好的共识案例，可以使用该区块共识算法。


#### The Eater Address

为了烧掉金币，用户发送将金币发送给 地址。一个 地址并没有任何私钥。因此，没有用户可以一直访问这些地址来发送金币。而且，这些地址由随机 生成。

尽管这些金币不可访问或者永远消失了，但是他们被提供给计算和燃烧标签？


#### 燃烧证明的利与弊

燃烧金币背后的主要原因是为了创造更好的稳定性。我们知道长期玩家往往持有金币很长时间来获利。

该系统青睐那些长期的投资者来基于更多稳定的货币流通，和长期提交？而且，这增强了去中心化并且可以创建更好的分布式网络。

但是无论你从那个角度来看该场景，燃烧金币就意味着浪费了它。甚至一些 地址在这里拥有超过100000美元价值的比特币。这里没有方法可以恢复这些金币，他们只能被烧掉。



### Proof of Weight

好，PoWeight区块链共识协议是我们列表上的最后一个共识算法了。它是PoS的大升级版本。在PoS中，你的钱越多，你有更好的机会发现更多区块。该些想法使得系统有很大的偏见。

好，PoWeight试图解决这个PoS中本来的偏见。想Algorand，Filecoin和Chia这样的加密货币都是PoWeight实现的。PoWeight考虑了比在PoS中拥有金币的一些因素？

这些因素被认为是权重因素。例如，Filecoin考虑了IPFS数据的账户，你可以拥有和计算这些因素的权重。一些其他的因素包括但不限于PoSpacetime和PoReputation共识协议。

该系统的基础优势包括可自定义和可拓展性。尽管对该共识算法来说激励是一个很大的挑战。
...
| PoWeight | Filecoin | 2017 | SNARK/STARK | 是 | 可拓展；自定义 | 激励问题 |
