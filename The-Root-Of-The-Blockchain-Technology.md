...
SBFT用于网络中保密性占优先地位的私有网络。该平台专为敏感设计但是有一定的限制。这就是为什么系统使用三种类型的技术了。例如，零知识证明，一次性地址是使用，加密元数据。

* 一次性使用地址

任何时刻用户想要从TA的钱包中接受资产，系统将为他们安排一个临时地址使用。所安排的每个地址都不相同，因此，保护了任何用户 交易。

* 零知识验证

零知识证明用于 交易的组成。但是，整个网络可能仍然需要综合验证。在零知识证明的帮助下这将完成，其中的一部分向另一部分证明他们的真实性。

在这种方式下，只有接收者和发送者才能看到交易的组成。

* 元数据加密

为了更进一步的保障安全性，交易的元数据会被加密。网络允许使用密钥来验证真实性。但是，为了更好的保护，密钥将在2到3天更换一次。

因此，他们将保持分离状态，并且在数据网络上的不同部分。所以，如果他们其中一个被入侵，还可以使用其他人的密钥来生成唯一的密钥。为了确保这些共识算法的整体性，每隔几天就需要管理和调整这些密钥。

链，就是基于平台的使用SBFT来验证网络上的所有交易的区块链。另一方面，他们还使用HSM来达到工业安全等级。通过使用HSM，他们确保在无需任何单个节点故障的情况下仍然安全。


### dBFT

毫无争议的事实是PoW和PoS是广为人知的共识算法。当很多区块链生态系统使用这两种通用的算法时，一些尝试将推动新的更高级的共识机制出现。在这系统区块链种类开拓者之间，NEO的名字慢慢出现在人们的视野。

在过去一年里繁荣的发展中，NEO现在时工业领域的抢手菜。中国的品牌展现出了相当的潜力。但为什么是他们呢？因为他们是高级共识定理的发明者--委托拜占庭容错dBFT。

#### NEO：受欢迎的区块链技术

NEO是现在市场上最受欢迎的加密货币之一。某段时间内它被认为是中国的“以太坊”。该网络主要集中于创建可以以低价分享你的数字资产的智能经济模型。

NEO使用dBFT来验证所有的交易。如果你在你的NEO中参与赌注，你将能生成GAS。GAS是该平台的主要流通货币。你将必须为每笔交易支付一定量的GAS费用。这是为什么你有囤有越多的NEO，你将获得更多的GAS的原因了。

但是，参与赌注和PoS相比有点不同。

许多交换提供一个交易池系统。但是，最好的方式是使用NEO官方的钱包而不是其他的存储钱包。

在我们分析dBFT之前，我必须让你门知道这个算法的继承算法遗留下的一些问题--拜占庭容错共识算法。


#### 拜占庭将军问题的缺点！

系统的主要缺点我们会在它身上见证任何类型的投票和结果。但是是怎样的方式呢？为了更好的理解这个缺点，你需要掌握一个算法的例子。

你已经知道在dBFT共识算法中的节点被认为敌人。一个敌意的节点有单个的将军，并且他们总是听从将军的指令。

现在想象一下，拜占庭敌军队打算攻打罗马并且拿下它。让我们仔细考虑下这有9个将军在拜占庭军队中，并且这些将军围绕着城邦准备发起攻击。他们可以攻下罗马，只要将军们听从统一的、单独的策略进行攻击或者撤退。

这里有个疑问。将军们有着独特的本质--他们听从51%的投票结果。但这里有两个被歪曲的计划。将军们不会坐在圆桌上讨论决定。相反，他们在不同的位置上互相传递消息。

#### 四个威胁！

这里有四个可能的方式可以帮助罗马保留王位。

第一，罗马可以试图贿赂将军们并给他们想要的东西。那些拿了贿赂的将军会被认为是“背叛者将军”。

第二，任何一个将军都会做出错误的决定来对抗集体的意愿。这些将军更为人知的是“功能不合理地将军”。

第三，信使或者 可能被罗马贿赂并传送错误决定给其他将军。

最后，第四点，罗马可以杀死 或者信使来扰乱将军之间的网络交流。

所以，拜占庭容错有四个重要的错误会使该共识算法不完美。


#### dBFT是如何改变场景的？

别保密？NEO给我们展示了一个更好的方式来解决拜占庭将军问题。现在让我们看看委托拜占庭容错--这是NEO引以为豪的共识机制。dBFT有两种方式主要集中于解决已有模型--更好的稳定性和更好的性能。


#### 发言人和代表们！

我们将使用另一个例子来理清dBFT模型。让我们考虑下拜占庭军队有选举领导者的权利而不是官僚的将军。这个被选择的领导者将实行军队代理。

你可能认为将军们会被这些选举的民主代表取代。甚至军队可以不同意这些代表或者选举另一个新的代表来替换先前的那个。

该举措限制了将军们的权利，并且没有将军会背叛整个军队。所以，罗马无法仅仅贿赂和买通将军来为他们工作了。

在dBFT中，被选举的代表必须保持对个体节点的决定的追踪。一个去中心化的账本记录所有这些节点的决定。

节点所在的军队也选举发言人来分享他们公共的、未知的想法给代表。为了通过新的法案，发言人分享军队中节点们的意见给代表们，至少要有66%的代表会同意该动机。否则，提议失败。

如果该动机没有得到代表们的66%的同意，提议将被拒绝，而新的动机继续会被提议知道达到共识为止。这个过程可以使军队免受叛徒或者将军们的背叛。

#### 不诚实的发言人们

这里仍然有两个可能的场景会阻碍dBFT区块链共识算法的整体协议--不诚实的发言人和不诚实的代表。

dBFT区块链共识协议也会给出我们解决这些场景的方法。正如我们所说，一个账本保留了某个地方所有节点的决定。代表们会验证发言人所描述的是否正实。如果发言人提议和账本没有联合？66%的代表会拒绝发言人的提议并且禁止该发言人。


#### 不诚实的代表们

另一个场景是有一个不诚实的发言人和可能背叛的代表。这里，诚实的代表和诚实的发言人将会试着质疑66%的大多数和减少不诚实代表的效率。

因此，你可以看到dBFT是如何从拜占庭将军和BFT共识算法中获得缺陷结果的。诚然，NEO为了更有效率的创建更好的共识算法从世界各地保留了 。
...
许多加密专家认为比特币是区块链1.0版本，以太坊是区块链的2.0版本。但是现在，我们可以看到市场上的一个新的玩家有着很多现代的技术。

一些人还说那是区块链3.0.当许多 努力想要知道区块链3.0的名字的时候，NXT将开始准备与我们熟知的DAG玩一场游戏了。抛开NXT不说，IOTA和IoT链也在他们的系统中使用了DAG。
...
| DPoS | Lisk | 2016 | JS | 否 | 能效；稳定；安全性 | 部分中心化；双花攻击 |
...
| DBFT | NEO | 2016 | Python/.NET/Java/C++/C/Go/Kotlin/JS | 是 | 稳定；快速 | 在链中有冲突产生 |
| DAG | IOTA | 2015 | JS/Rust/Java/Go/C++ | 确认中 | 低成本网络；稳定性 | 实现鸿沟；不适合只能合约 |
...
| PoWeight | Filecoin | 2017 | SNARK/STARK | 是 | 稳定；自定义 | 激励问题 |
